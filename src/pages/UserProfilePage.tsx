        ...
        <div style={{ maxWidth: 700, margin: '0 auto', padding: '48px 24px 24px 24px', width: '100%' }}>
          <div className={styles['profile-card']}>
            <Avatar
              ...
              size={128}
            />
            <div className={styles['profile-title']}>{name || email || 'User'} Profile</div>
            {createdAt && (
              <div style={{ fontSize: tokens.fontSizeBase200, color: 'var(--fluent-colorNeutralForeground3, #888)', marginBottom: tokens.spacingVerticalXS, marginTop: tokens.spacingVerticalXS, textAlign: 'center' }}>
                Member since: {new Date(createdAt).toLocaleString(undefined, { month: 'long', year: 'numeric' })}
              </div>
            )}
            <form className={styles['profile-form']} onSubmit={(e: any) => { e.preventDefault(); handleSave(); }}>
              ...
              <Input id="profile-password" type="password" value={password} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)} placeholder="Leave blank to keep current password" autoComplete="new-password" />
              <Label htmlFor="profile-confirm-password">Confirm Password</Label>
              <Input id="profile-confirm-password" type="password" value={confirmPassword} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setConfirmPassword(e.target.value)} placeholder="Repeat new password" autoComplete="new-password" />
              ...
              <Button appearance="subtle" type="button" onClick={() => window.location.href = '/dashboard'} icon={<ArrowLeft24Regular />}>Back</Button>
              <Button appearance="primary" type="submit" disabled={loading} icon={loading ? <Spinner size="tiny" /> : <Save24Regular />}>Save Changes</Button>
              </div>
            </form>
          </div>
        </div>
